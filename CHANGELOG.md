...
- [#183](https://github.com/nf-core/proteinfamilies/pull/183) - Restricted samplesheet sample names to letters, digits, dots, underscores and dashes. Family names are parsed back out of file names with `split("<sample>_")`, which takes a regex, so a sample name carrying a regex metacharacter could mis-split; the sample name also builds output paths. (by @vagkaratzas, raised by @erikrikarddaniel)
